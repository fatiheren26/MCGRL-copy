...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    for image, label in dataloader:
        if train and count % 100 == 0:
            print(f"Epoch {epoch+1} iteration {count}/{total}...")
        count += 1
        if train:
            optimizer.zero_grad()
        ...
        idx_list = []
        h_a, h_p, _, pred = model(image)  # h_a, h_p: (batch_size, 512)
        batch_size_curr = h_a.shape[0]
        
        # Build a KNN graph between images in the batch based on h_a features
        with torch.no_grad():
            # h_a: (B, 512) -> (B, 512, 1)
            # pairwise_distance expects (B, N, D) or similar. 
            # Our xy_pairwise_distance expects (B, N, D)
            # Let's just use a simple one:
            dist = torch.cdist(h_a, h_a) # (B, B)
            k_knn = min(num_neg + 1, batch_size_curr)
            _, nn_idx = torch.topk(-dist, k=k_knn) # (B, k)
            
            # Convert to edge_index format (2, B*k)
            # Source nodes (neighbors)
            row = nn_idx.flatten()
            # Target nodes (center)
            col = torch.arange(batch_size_curr, device=h_a.device).view(-1, 1).repeat(1, k_knn).flatten()
            edge_index_flat = torch.stack((row, col), dim=0) # (2, B*k)
            
            nb_nodes = batch_size_curr
            A_degree = [k_knn] * nb_nodes # Every node has k_knn neighbors in this dense setup
        ...
        idx_p_list = []
        # edge_index_flat[0] is neighbors
        neighbors_flat = edge_index_flat[0]
        for j in range(1, 102):
            random_list = [deg_list_2[i] + j % A_degree[i] for i in range(nb_nodes)]
            idx_p = neighbors_flat[random_list]
            idx_p_list.append(idx_p)

        idx_list = []
        for i in range(num_neg):
            idx_0 = torch.randperm(nb_nodes, device=h_a.device)
        ...
        for h_n_idx in idx_list:
            s_n = F.pairwise_distance(h_a, h_a[h_n_idx])
            s_n_list.append(s_n)
        ...
        mask_margin_N = mask_margin_N / (num_neg * batch_size_curr)
...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    for image, label in dataloader:
        ...
        result = model(image)
        # SUGRL_Fast returns (h_a, h_p, edge_index, pred)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='MORPH', help='dataset name')
    ...
    for e in range(n_epochs):
        ...
        train_mse_loss, train_preds, train_labels, train_cs = train_or_eval_graph_model(model, loss_function,
                                                                              train_loader, e, optimizer, True)
        valid_mse_loss, valid_preds, val_labels, valid_cs = train_or_eval_graph_model(model, loss_function, valid_loader, e)

        if best_loss == None or best_loss > valid_mse_loss:
            best_loss, best_labels, best_preds = valid_mse_loss, val_labels, valid_preds
            torch.save(model.state_dict(), 'best_mcgrl_morph.pth')
            print(f"Saved best model with MAE: {best_loss}")

        if (e + 1) % 5 == 0:
            torch.save(model.state_dict(), f'mcgrl_morph_epoch_{e+1}.pth')
            print(f"Saved checkpoint for epoch {e+1}")
