...
def get_MORPH_images_and_labels(dir_path):
    ...
    dir_path = Path(dir_path)
    count = 0
    for img_path in dir_path.rglob('*.[jJ][pP][gG]'):
        images_list.append(str(img_path))
        labels_list.append(int(str(img_path.name)[-6:-4]))
        count += 1
        if count % 1000 == 0:
            print(f"Found {count} images...")
    print(f"Found total {len(images_list)} images.")
    return images_list, labels_list
...
class MORPHDataset(Dataset):
    def __init__(self, dir_path=r'c:\Users\fatih\OneDrive\Masaüstü\age_detection\Dataset\Images\Train', transform=MORPH_transform):
        print(f"Initializing MORPHDataset with path: {dir_path}")
    ...
    def __getitem__(self, index):
        ...
        # Use numpy.fromfile + cv2.imdecode to handle Unicode paths (e.g. Turkish characters)
        img = cv2.imdecode(np.fromfile(img_path, dtype=np.uint8), cv2.IMREAD_COLOR)
        if img is None:
            # Skip unreadable images by trying the next one
